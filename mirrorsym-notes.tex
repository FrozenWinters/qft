...
In this subsection, $G$ is a topological group acting smoothly on $X$.
All cohomology is taken with coefficients in $\bQ$.

The usual cohomology theories fail to capture any information about
this action. Equivariant cohomology is an extension of the usual
cohomology for $X/G$ in order to account for the action. For example,
if $G$ acts smoothly without fixed points, then $X/G$ is a smooth
manifold, and equivariant cohomology agrees with $H^*(X/G)$. On the
other end of the spectrum, if $X = \{\pt\}$, then any $G$-action fixes
$\pt$, and we want equivariant cohomology to give a cohomological
invariant of $G$. To construct equivariant cohomology we need the
machinery of classifying spaces.
...
smooth manifolds are homotopy equivalent to CW-complexes. \todo{Cite
  Milnor's Morse theory monograph for this; it's in there somewhere.}
...
\begin{theorem}[Milnor] \label{thm:classifying-space}
...
\begin{proof}
  \todo{Outline proof for $\GL(n)$ using the infinite Stiefel manifold
    over the infinite Grassmannian, and therefore the proof for matrix
    Lie groups. Then do $K(G, 1)$ for discrete groups.}
\end{proof}
...
\begin{definition}
  Let $E$ be a $G$-bundle over $X$ (which has a $G$-action). For a
  point $x \in X$, let $G_x \coloneqq \{g : gx = x\}$ be the
  stabilizer. Define the product
  \[ E \times_G X = E \times X / \sim, \quad (eg, x) \sim (e, gx) \quad \forall e \in E, x \in X, g \in G, \]
  i.e. the fibered space over $X/G$ with the fiber over $[Gx]$ equal
  to $E/G_x$.
\end{definition}
...
\begin{example}
  ...
  H^*(BG)$. The equivariant map $X \to \{\pt\}$ induces an
  $H^*(BG)$-module structure on $H^*_G(X)$ for any $X$. Since this
  situation is important and comes up often, we have the notation
  $H^*_G \coloneqq H^*_G(\{\pt\}) = H^*(BG)$.
\end{example}
...
\begin{example}
  ...
  one) to get the universal principal $\bC^*$-bundle $\bC^\infty \to
  \bC\bP^\infty$. Hence $BS^1 = \bC\bP^\infty$, and
...
\begin{definition}
  A smooth {\bf $G$-equivariant form} on $X$ is a $G$-equivariant map
  $\alpha\colon \fg \to \Omega^*(X)$, i.e.
  \[ \alpha(Z) = g^{-1} \alpha(\Ad(g)Z) \quad \forall Z \in \fg, \, g \in G. \]
  Equivalently, it is a $G$-invariant element of $\Sym(\fg^*) \otimes
  \Omega^*(X)$. The space of $G$-equivariant forms is denoted
  $\Omega^*_G(X)$.
\end{definition}

\begin{definition}
  The {\bf equivariant exterior derivative} $d_{\fg}\colon
  \Omega^{\text{odd}}_G(X) \leftrightarrow \Omega^{\text{even}}_G(X)$
  is given by
  \[ d_{\fg}\alpha\colon \fg \to \Omega^*(X), \quad (d_{\fg}\alpha)(Z) \coloneqq d(\alpha(Z)) - \iota_{Z^\#}(\alpha(Z)) \]
  where $d$ is the usual exterior derivative, $\iota$ is contraction,
  and $Z^\#$ is the fundamental vector field associated to $Z \in
  \fg$.
\end{definition}

\todo{Finish off exposition of de Rham model for equivariant
  cohomology.}
...
\begin{corollary} \label{thm:chi-genus-formula}
  ...
  \[ \chi_y = \int_X \prod_{k=1}^n (1 + y \exp(-a_k)) \frac{a_k}{1 - \exp(-a_k)} = \int_X \prod_{k=1}^n a_k \coth(a_k/2). \]
\end{corollary}
...
\begin{definition}
  If $i\colon V \to X$ is a map of compact manifolds, then there is a
  {\bf pushforward} or {\bf wrong way map} on cohomology $i_*\colon
  H^*(V) \to H^{*+k}(X)$: use Poincar\'e duality on $V$, push forward
  the homology cycle, then use Poincar\'e duality on $X$.
\end{definition}

\begin{example}
  Let $i\colon V \to X$ be an inclusion of compact manifolds and let
  $k$ be the codimension of $V$. Then $i_*1 = \Phi(NV)$, the Thom form
  of the normal bundle of $V$. This is straightforward to check: the
  Poincar\'e dual of $1$ is all of $V$, which sits in $X$ by the
  inclusion $i$, which is dual, fiberwise, to the preferred generator
  on each fiber, which is precisely the Thom class. The Thom form
  $\Phi(NV)$ can be extended by zero so that it lives in $H^k(X)$, and
  the pullback $i^*\colon H^k(X) \to H^k(V)$ then gives $i^*\Phi(NV) =
  e(NV)$ (c.f. the definition \ref{def:euler-class} of the Euler
  class). Hence
  \[ i^* i_* 1 = e(NV). \]
  The composition $i^*i_*$ is called the {\bf Gysin operation}.
\end{example}
...
in equivariant cohomology we can.
...
\begin{definition}
  ...
  equivariant vector bundle $E$ induces a vector bundle $\tilde{E}$ on
  the homotopy quotient $EG \times_G M$. An {\bf equivariant
    characteristic class} of $E$ is an ordinary characteristic class
  of $\tilde{E}$, which is an element of $H^*(EG \times_G X) =
  H^*_G(X)$.
\end{definition}
...
\begin{proof}
  ...
  H^*_{\bT}$ is a non-zero polynomial. Some work shows that this
  component is always non-zero. \todo{Cite or use Atiyah's paper ``The
    Moment Map and Equivariant Cohomology''.}
\end{proof}

\begin{corollary}[Atiyah--Bott localization formula] \label{thm:equiv-localization}
  For $\phi \in H_{\bT}^*(X)$ and working in
  $H^*_{\bT}(X)[(H^*_{\bT})^{-1}]$,
  \[ \phi = \sum_i i^*\left(\frac{i^* \phi}{e(NF_i)}\right). \]
\end{corollary}

To connect this formula to our previous notion of localization
formulas, where we recover global information from local information
at fixed points, we usually write it as
\[ \int_M \phi = \sum_i \int_{F_i} \frac{i^*\phi}{e(NF_i)}. \]
(This follows by recalling that $i_*$ is defined via Poincar\'e
duality.) This formula is very powerful, as we shall see.

\begin{proposition}
  Let $\bT$ act on $X$ with fixed point loci $\bigcup_i F_i$. Then
  \[ \chi(X) = \sum_i \chi(F_i). \]
\end{proposition}

\begin{proof}
  The key is that the action of $\bT$ on $X$ lifts to an action of
  $\bT$ on $TX$. \todo{Finish this proof.}
\end{proof}
