...
Hamiltonian, solutions) you want to quantize, and impose {\bf
  canonical commutation relations} modeled on $[x, p] = i$.
...
be reinserted via dimensional analysis.
...
where $p_i$ is the momentum conjugate to $x^-$, and $\Pi^i$ is the
...
Fourier expand:
\begin{align*}
  X^i_L(\sigma^+) &= \frac{1}{2}x^i + \frac{1}{2\pi T_0} p^i \sigma^+ + i\sqrt{\frac{1}{4\pi T_0}} \sum_{n \neq 0} \frac{1}{n} \tilde{\alpha}_n^i e^{-in\sigma^+}, \\
  X^i_R(\sigma^-) &= \frac{1}{2}x^i + \frac{1}{2\pi T_0} p^i \sigma^+ + i\sqrt{\frac{1}{4\pi T_0}} \sum_{n \neq 0} \frac{1}{n} \alpha_n^i e^{-in\sigma^-}.
\end{align*}
...
factors of terms.) Because the $X^i$ are real fields, we have the
{\bf constraints} $\tilde{\alpha}^i_n = (\tilde{\alpha}^i_{-n})^*$ and
$\alpha^i_n = (\alpha^i_{-n})^*$ on the Fourier coefficients, along with the Virasoro conditions 
...
\[ [x^i, p^j] = i\delta^\mu{}_\nu, \quad [\alpha^i_m, \alpha^j_n] = [\tilde{\alpha}^i_m, \tilde{\alpha}^j_n] = m \delta^{ij} \delta_{m+n,0}, \]
with all other commutators vanishing. So as in QFT, we can treat
$\tilde{\alpha}^i_m$ and $\alpha^i_n$ as creation and annihilation
operators, and build up our state space using them. Note that instead
of just two operators, we have two infinite towers of them! Without
loss of generality, we are concerned only with $\alpha^i_n$, which are
operators for right-moving modes.
...
$N_{i,n}$ for $i = 2, \ldots, D-1$ and $n = 1, 2, \ldots$:
\[ \ket{N; k} \coloneqq \left(\prod_{i=2}^{D-1} \prod_{n=1}^\infty \frac{(\alpha^i_{-n})^{N_{i,n}}}{\sqrt{n^{N_{i,n}} N_{i,n}!}}\right) \ket{0; k}. \]
...
\begin{definition}
  ...
  is an exterior algebra.)
\end{definition}

(\CHECK{} What happened to the Virasoro conditions?)

\subsection{Spectrum and Critical Dimension}

Polchinski pages 21.5 - 23, Tong pages 38 - 42.
...
\item Because $b$ is a symmetric $2$-tensor (do {\bf not} confuse the
  ...
  rewrite
  \[ b^{ab}(\nabla_a c_b + \nabla_b c_a) = b^{ab}\nabla_a c_b + b^{ab} \nabla_a c_b = 2b^{ab}\nabla_a c_b. \]
\item We also rewrote $b^{ab}\nabla_a c_b = b_{ab} \nabla^a c^b$.
\item We chose slightly different normalization factors to make later
  computations cleaner. These factors make absolutely no difference,
  because we can absorb them into the fields $b$ and $c$.
\end{enumerate}
The quantity $S_{\text{G}}$ is called the {\bf ghost action}, because
when we plug $\detFP(\hat{g})$ back into the path integral, we get
\[ Z = \int \cD X \, \cD b \, \cD c \, \exp(-S_{\text{P}}[X, \hat{g}] - S_{\text{G}}[b, c]), \]
i.e. $S_{\text{G}}$ becomes part of the action. The fields $b$ and
$c$, which do not correspond physically to anything, are known as {\bf
