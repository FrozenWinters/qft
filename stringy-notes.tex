...
\begin{definition}
  ...
  choice is {\bf lightcone gauge}, given by $X^+ = R\sigma^0$, where
  \[ X^\pm \coloneqq \frac{1}{\sqrt{2}}(X^0 \pm X^{D-1}) \]
  are {\bf lightcone coordinates}.
\end{definition}

Clearly neither static gauge nor lightcone gauge exhausts the gauge
...
lightcone gauge. Let's take a different approach and try to transform
the metric first.
...
\begin{exercise} (Important!)
  ...
  static gauge that for an open string with free endpoints, i.e.
  endpoints satisfying the Neumann boundary condition, $|\di_0\vec{X}|
  = 1$. Conclude that string endpoints always move at the speed of
  light.
\end{exercise}
...
representative of each gauge orbit. But, perhaps unexpectedly, there
is more gauge freedom: there are non-trivial transformations in $\diff
\times \Weyl$ that preserve unit gauge! The key to finding these
transformations is to realize that $\Sigma$ is actually a {\bf Riemann
  surface}: let $z \coloneqq \sigma^0 + i\sigma^1$, so that $ds^2 = dz
d\bar{z}$. Now if $f(z)$ is a holomorphic change of coordinates, then
...
\begin{enumerate}
\item take the classical object (e.g. Lagrangian, Hamiltonian,
  solutions) you want to quantize, and
\item impose {\bf canonical commutation relations} modeled on $[x, p]
  = i$.
\end{enumerate}

\subsection{Spectrum of Open String}
