...
include Polchinski's Vol 1, Polchinksi's Little Book, and Ginsparg's
...
the \textbf{operator product expansion} (OPE). This is a canonical form
...
world-sheet field theory with $X_1(\sg),\ldots,X_n(\sg)$ be fields. 
A \textbf{local functional} or \textbf{local operator} is a function $\ms
F[X]$ taking in, as arguments, $X_i(\sg_0)$ and $\di_a X_i(\sg_0)$ which
are taken at $\sg_0$.
\end{definition}
...
\begin{definition} Let $\ms F$ be an arbitrary function of $X$. Define the
(free-field) normal order of $\ms F$ to be the functional:
\[ \normalorder{\ms F} = \exp \left( -\FR{1}{8\pi T}\int d^2z_1d^2z_2 \ln|z_{12}|^2
