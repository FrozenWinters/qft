...
\[ N_m^\mu \coloneqq \alpha^\mu_{-n} \alpha^\mu_n, \quad N_m^b \coloneqq nb_{-n} c_n, \quad N_m^c \coloneqq nc_{-n} b_n, \quad N \coloneqq \sum_{m=1}^\infty \left(\sum_{\mu=0}^{25} N_m^\mu + N_m^b + N_m^c\right). \]
...
states are distinguished by the sign of $k^0$.)
...
\begin{definition}
  An {\bf $n$-string S-matrix element} involving $n$ strings with
...
\[ \Delta_{\text{FP}}(\hat{g}, \hat{\sigma})^{-1} = n_R \int d^\tau (\delta m) \, \cD(\delta \omega) \, \cD(\delta \sigma) \, \delta(\delta g_{ab}) \prod_{(a,i) \in \Fixed} \delta(\delta \sigma^a(\hat{\sigma}_i)), \]
...
\[ \Delta_{\text{FP}}(\hat{g}, \hat{\sigma}) = \frac{1}{n_R} \int \cD b \, \cD c \, d^\tau \zeta \, d^\kappa \eta \, \exp\left(-\frac{1}{4\pi} (b, 2P_1 c - \zeta^j \di_j \hat{g})_{\hat{g}} + \sum_{(a,i) \in \Fixed} \eta_{ai} c^a(\hat{\sigma}_i)\right). \]
...
\[ \Delta_{\text{FP}}(\hat{g}, \hat{\sigma}) = \frac{1}{n_R} \int \cD b \, \cD c \, \exp(-S_{\text{G}}) \prod_{j=1}^\tau \frac{(b, \di_j \hat{g})_{\hat{g}}}{4\pi} \prod_{(a,i) \in \Fixed} c^a(\hat{\sigma}_i) \]
...
These eigenbases are related. Note that $P_1\cC_J$ is an eigenfunction
of $P_1P_1^\dag$, and similarly, $P_1\cB_K$ is an eigenfunction of
$P_1^\dag P_1$. Hence there is a one-to-one correspondence between the
eigenbases $\{\cC_J\}$ and $\{\cB_K\}$ except for when $P_1\cC_J = 0$
or $P_1\cB_K = 0$, i.e. when the eigenfunction has a zero eigenvalue.
The $\cC_J$ with zero eigenvalue correspond to $\kappa$ CKVs, and the
$\cB_K$ with zero eigenvalue correspond to $\tau$ moduli. Denote these
eigenfunctions of zero eigenvalue $\{(\cC_{0j})^a\}_{j=1}^\kappa$ and
...
\exp(-x^2) x^k$, we get
\[ \Delta_{\text{FP}} = \left[\det\left(\frac{(\cB_{0k}, \di_{k'}\hat{g})}{4\pi}\right)_{k,k'=1}^\tau\right]\left[\det\left((\cC_{0j})^a(\sigma_i)\right)_{j=1, (a,i) \in \Fixed}^\kappa\right] \left[\det' \left(\frac{P_1^\dag P_1}{2\pi}\right)^{1/2}\right], \]
...
It remains to handle the integration
...
introduce a formal variable $J$, write down the {\bf generating
  functional} $Z[J]$, and do tricks with $J$ in order to get the
vertex operator insertions we want.

\section{Tree-Level Amplitudes}

For closed strings, tree-level amplitudes correspond to $\Sigma = S^2$
with marked points. Hence we must understand $\cM(S^2)$ and
$\CKG(S^2)$.
...
\begin{proof}
  ...
  equivalent. However, for CKVs, we only need $\delta z \propto z^2$
  as $|z| \to \infty$, so in general,
