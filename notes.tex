...
\begin{exercise}
  Symmetry factors take some getting used to. Compute the symmetry factors of the following three diagrams:
  \begin{center}
  \begin{minipage}{0.3\textwidth}
    \centering
    \begin{tikzpicture}[node distance=1.5cm and 1.5cm]
...
\begin{exercise}
  ...
  \begin{itemize}
  \item for each propagator, label it with a momentum $p$ and add a
    $i/(p^2 - m^2 + i\epsilon)$ term;
  \item for each internal vertex, add a $-i\lambda$ term;
  \item for each external vertex with a propagator of momentum $p$
