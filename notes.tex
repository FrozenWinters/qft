...
the Lagrangian explicitly in terms of the $A_\mu$:

\section{Aside: Scattering Amplitudes}
% discuss cross section informally
...
\subsubsection{Computing scattering amplitude using cross sections}
...
\label{Smatrix}
\end{align}
The zeros on the end of the bracket refer to idealistic wavepackets. Our
next goal will be to interpret this amplitude as a sum of Feynman diagrams.
...
\begin{itemize}
    \item (4.96-4.103)
    \item Trick: exponentiation of vacuum bubbles $\equiv$ to finding the
        phase factor for $\ket\Omega \propto \ket0$.
        Similarly the amputations on external legs comes from the $0$ in
        the formular for the $S$-matrix, \eqref{Smatrix}.
    \item Write down Feynman rules for Bosons, Fermions, EM.
    \item Compute one example, if running out of time, end off with:
    \item Do Coulomb potential (4.133-4.136)
\end{itemize}
...
$$ \cM = \frac{(-i\lambda)^2}{2} \int \frac{d^4p}{(2\pi)^4} \frac{i}{p^2 - m^2 + i\epsilon} \frac{i}{(p_1 + p_2 - p)^2 - m^2 + i\epsilon}. $$
...
amplitude is
$$ \cM = -i\lambda + \frac{i\lambda^2}{32\pi^2}\left[\log\frac{\Lambda^2}{s} + \log\frac{\Lambda^2}{t} + \log\frac{\Lambda^2}{u}\right] + O(\lambda^3). $$
...
given by
$$ \cM = \frac{(-i\lambda)^2}{2} \int \frac{d^4p}{(2\pi)^4} \frac{i}{p^2 - m^2 + i\epsilon} \frac{i}{(p+q)^2 - m^2 + i\epsilon}. $$
...
$$ \cM = \frac{(-i\lambda)^2}{2} \int \frac{d^4p}{(2\pi)^4} \frac{-i}{(|p|^2 + m^2)(|p + q|^2 + m^2)}, $$
...
$$ \cM = \frac{(-i\lambda)^2}{2} \int \frac{d^4p}{(2\pi)^4} \int_0^1 dx \, \frac{i}{(|p|^2 + 2x p \cdot q + x|q|^2 + m^2)^2}. $$
...
suffices:
$$ \cM = \frac{(-i\lambda)^2}{2} \int_0^1 dx \, \int \frac{d^4p}{(2\pi)^4} \frac{i}{(|k|^2 + x(1 - x)|q|^2 + m^2)^2}. $$
...
integral for $\cM$, which we call $I_d(q)$,
$$ \cM = I_d(q) = -i\frac{(-i\lambda)^2}{2} \frac{\Gamma(2 - d/2)}{(4\pi)^{d/2}} \int_0^1 \frac{dx}{(m^2 - x(1-x)q^2)^{(4-d)/2}}. $$
